...
| --- | --- | --- |
| TTFB | ≥ 1.0s | server timing (any HTTP response) |
...
## The LLM: a scoped pointer, never the score

The deploy and grade pipeline uses an LLM, and it is fenced off from the number. The rule is simple. The
LLM decides where to look and flags what was missed. It never decides slop. Two passes, both in
`scripts/deploy_and_grade.py`.

- **Perception (`perceive_surface`).** The LLM reads the rendered page and points at surface a static crawl
  misses: a client rendered login, an upload widget, an action button, a likely API path. Proactive
  discovery folds those hints into the profile (`discovery.merge_perceived`), tagged `origin="llm"`, and
  the deterministic probes then test them. A hint that is wrong makes a probe fire on nothing, which
  no-ops, so a bad pointer only ever discovers less. It can never turn one app into two different scores.
- **Coverage audit (`audit_coverage`).** A second pass reads the live page and reports what discovery did
  not reach, plus the page state (working, login wall, broken, placeholder, not an app). It rides beside
  the score as a flag for a human and feeds the DNF gate. It never subtracts a point itself.

Determinism does not come from the model being steady. It comes from temp-0 plus a per commit cache of the
whole discovered surface. The first grade of a commit mints the surface, and every later grade of the same
commit replays it and never calls the model again. That is what makes an appeal or a re-run reproducible.

Reasoning is off by default on both passes (`reasoning: {enabled: false}` on OpenRouter). These are
extraction and classification jobs, not deep reasoning, and turning thinking off cut the audit call from
about 36s to under 10s and removed its dominant token cost with no change to the score, the precision, or
the page state classification (a paired A/B on 1043 apps, median delta 0). `--llm-reasoning` turns it back
on. The lever that works on this model is `reasoning: {enabled: false}`. The obvious
`chat_template_kwargs.enable_thinking` is silently ignored by the provider and does nothing.
...
The runner explores the submission to build its profile. Discovery is not a plain crawl. It renders the
app in a real browser and folds in the LLM perception pass, so it reaches a client rendered surface a
static crawl never sees.

1. Start at the homepage (configurable per submission, typically `http://submission:PORT/`).
2. Crawl reachable URLs by HTML link extraction, and render the head routes in a headless browser
   (Playwright) so client rendered forms, controls, and routes appear. Interaction clicks a bounded set of
   create style controls (New Board, Add Card) that a static crawl leaves invisible.
3. Identify form endpoints by parsing `<form>` elements in the rendered DOM.
4. Identify API endpoints by common patterns (`/api/*`, GraphQL), by ingesting an OpenAPI or Swagger spec
   if present, and by mining API paths out of the SPA's JS bundles.
5. Run the LLM perception pass (`perceive_surface`): it reads the rendered page and points at surface the
   crawl missed (a client rendered login, an upload widget, a likely API path). Those hints fold into the
   profile tagged `origin="llm"` and are then tested by the deterministic probes. A wrong hint no-ops. See
   "The LLM: a scoped pointer, never the score".
6. Identify authentication boundaries from 401/403 responses, and self register two accounts where the app
   allows it (driving a browser when the app authenticates by a JWT in localStorage rather than a cookie).
7. Identify file upload capabilities from form encoding types and file inputs.
8. Determine whether the app serves a rendered HTML document versus a pure API. This gates the FCP and INP
   speed checks.
9. Establish per endpoint baselines (benign input signatures: status, length, timing) so the adversarial
   oracles have a reference to diff against, and baseline out endpoints that 500 only because a key is a
   dummy.
10. Produce the submission profile, and on the first grade of a commit, freeze it in the per commit surface
    cache so a re-grade replays it verbatim.

The discovery phase runs for a fixed time budget or until exhaustion, whichever comes first.
...
The central runner runs in a queue model. 12 submissions arrive at freeze, and workers process them in
parallel up to infrastructure capacity, with the target of completing all of them inside the 12 minute
evaluation window. The standalone scraping path uses a separate batch orchestrator (`scripts/run_batch.py`)
that grades many hackathons at once, balances the limit across the slugs, interleaves them breadth first so
a partial run stays a balanced sample, and grades URLs in parallel with a lock guarded append to the
results file.
