...
better). Canonical design: [FUZZ_RUNNER_SPEC.md](FUZZ_RUNNER_SPEC.md).

> **New here (human or agent)?** Start with **[CLAUDE.md](CLAUDE.md)** (operating manual),
> **[STATE.md](STATE.md)** (where the project stands + what's next), and **[PROJECT_LOG.md](PROJECT_LOG.md)**
> (how it got here). The rest of this README documents the *original vertical slice* and undersells
> current scope: the catalog is now **65 probes** across three bundles, discovery is browser-driven, and
> there's an LLM-assisted deploy+grade **batch** pipeline in `scripts/`.
