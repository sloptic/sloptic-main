...
_CONTENT: dict[str, tuple[str, str, str]] = {
    ...
    "sec-tls-001": ("The public origin is served over HTTPS.",
                    "No transport encryption: the site is served over plain `http://` with no upgrade to HTTPS, so every visitor's credentials and session cookies cross the network in the clear (trivially sniffed or injected).",
                    "Terminate TLS at the origin (a free certificate via your host or Let's Encrypt), redirect `http`->`https`, and set HSTS."),
    "sec-sri-001": ("Cross-origin scripts and styles load with a Subresource Integrity (`integrity`) hash.",
                    "A third-party (CDN) script or stylesheet loads without SRI, so a compromised or hijacked CDN can run arbitrary code in your origin -- a supply-chain foothold.",
                    "Add `integrity` + `crossorigin` to cross-origin `<script>`/`<link>` tags (or self-host the asset), pinned to the exact version you audited."),
    "sec-exposure-009": ("The client bundle exposes no internal-only infrastructure addresses.",
    ...
    "qa-input-002": ("International and multibyte text (emoji, CJK, Arabic, accents) round-trips intact and never crashes the app.",
                     "The app corrupts non-ASCII input (mojibake / `?` / replacement chars) or `500`s on it — a legacy encoding gap (a latin1 column, MySQL `utf8` instead of `utf8mb4`, or a naive byte-slice) that mangles or rejects real users' names and messages.",
                     "Use UTF-8 / `utf8mb4` end to end (DB columns, connection charset, form encoding) and slice on characters not bytes; test with emoji and CJK input."),
    "qa-email-001": ("The signup confirmation email arrives promptly and reliably, with a working resend option.",
                     "Email verification is unreliable — no confirmation email within 60s (even after a resend), or only a late one — so a user who signs up is locked out of the account they just created.",
                     "Fix the transactional-email path (correct provider key, a verified sender domain, handle send failures) and offer a working resend; confirm delivery end to end, not just that the send call returned."),
    "qa-email-002": ("Acting on the confirmation email (its link or code) actually logs the user in.",
                     "The confirmation email arrives but acting on it establishes no session — a dead link, an unrecognized token, or a verify step that no-ops — so the user did everything asked and still cannot get in.",
                     "Make the verification endpoint consume the token and establish (or enable) the session; test the full click-through, and keep tokens valid long enough to use."),
    "qa-reset-001": ("Password reset works: requesting one delivers an email whose link opens a working reset page.",
                     "Account recovery is broken — the reset email never arrives, or its link is dead — so a user who forgets their password is locked out for good.",
                     "Wire forgot-password to the same working email path as signup and make the reset link resolve to a live reset form; verify the whole round-trip."),
    "qa-deploy-002": ("Every route resolves without an endless redirect loop.",
                      "The homepage or a linked route redirects forever (`ERR_TOO_MANY_REDIRECTS`), so the page never loads for anyone — usually two rules (an auth guard, an `http`->`https` or trailing-slash rule) bouncing the request back and forth.",
                      "Find the two rules fighting and make one terminal; verify the redirect chain resolves to a real page."),
    "qa-deploy-003": ("OAuth sign-in redirects back to the deployed public origin.",
                      "The OAuth `redirect_uri` points at `localhost` / a private IP / an unset env var, so after authenticating the provider bounces the user to a host they cannot reach — sign-in is dead in production (works-on-my-machine).",
                      "Drive the OAuth redirect URI from the deploy environment (your public origin) and register that exact URI with the provider; never ship the localhost callback."),
